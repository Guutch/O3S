...
// Check connection to a Google Sheet by spreadsheet ID
...
router.post('/import-sheets/:campaignId', async (req, res) => {
  try {
    ...
    // Fetch the raw data from Google Sheets using the provided spreadsheetId
    ...
    // Normalize the headers (first row)
    const headers = values[0].map(header =>
      header.trim().toLowerCase().replace(/\s/g, '_')
    );
    console.log("Normalized headers:", headers);
  
    // Check required headers against your Leads schema
    ...
    for (const row of rows) {
      ...
      if (!lead) {
        ...
        lead = await Lead.create({
          // Use req.user.id if available, else default to 1
